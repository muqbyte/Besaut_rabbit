...
export default function ControllingFanTwo() {
  ...
  const toggleSwitch = async () => {
    ...
    try {
      ...
      const res = await axios.get(`https://nr.txio.live/ranch/control/ch2_${newStatus}`);
      console.log(res);
  ...
  const fetchStatus = async () => {
    try {
      const response = await fetch('http://13.250.36.154:5000/ranch/controlstatus/RC-02');
  ...
  return (
    <View style={{display:"flex", flexDirection:"row", justifyContent:"space-between",alignItems:"center",width:windowWidth/2}}>
        <View>
            <Controlling title={"RC-02"} status={status}/>
        </View>
      <Switch
